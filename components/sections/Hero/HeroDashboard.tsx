...
export function HeroDashboard() {
  ...
  return (
    <div ref={ref} className="relative mt-16 w-full" style={{ perspective: 1400 }}>
        ...
        <div className="flex h-[440px]">
                    ...
                    <div key={item}>
                      <div
                        className={`flex items-center gap-2 rounded-md px-2.5 py-1.5 text-[13px] ${
                          active ? "bg-primary-100 font-medium text-primary" : "text-muted"
                        }`}
                      >
                      ...
                      {active && (
                        <div className="ml-4 mt-0.5 space-y-0.5 border-l border-border pl-3 text-[12px]">
                          {["Individual", "Corporate", "Corporatives", "Clusters"].map((s) => (
                            <p key={s} className={s === "Individual" ? "text-primary" : "text-muted-2"}>
                              {s}
                            </p>
                          ))}
                        </div>
          ...
          <div className="flex-1 overflow-hidden p-5">
            <div className="mb-4 flex gap-5 border-b border-border text-[13px]">
              {["Client Overview", "Loan Account", "Pending", "Failed"].map((t, i) => (
                <span
                  key={t}
                  className={`pb-2.5 ${
                    i === 0 ? "border-b-2 border-primary font-medium text-fg" : "text-muted-2"
                  }`}
                >
                  {t}
                </span>
            ...
            <div className="mb-5 flex items-center gap-3 rounded-lg bg-surface-2 p-3">
              ...
              <div>
                <p className="text-sm font-semibold text-fg">Temitope Waniyon</p>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-4 lg:grid-cols-4">
              {fields.map(([label, value]) => (
                <div key={label}>
                  ...
                  <p className="truncate text-[13px] font-medium text-fg">{value}</p>
                </div>
              ))}
            </div>
          </div>
